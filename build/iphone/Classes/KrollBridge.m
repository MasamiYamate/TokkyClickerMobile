...
NSString * TokkyClickerMobile$ModuleRequireFormat = @"(function(exports){"
...
@implementation TokkyClickerMobileObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_tokkyclickermobile);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create TokkyClickerMobile global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "TokkyClickerMobile" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_tokkyclickermobile = [[TokkyClickerMobileObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_tokkyclickermobile];
	
	NSString *_tokkyclickermobileNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _tokkyclickermobileNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_tokkyclickermobile valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:TokkyClickerMobile$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.tokkyclickermobile.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.tokkyclickermobile.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
