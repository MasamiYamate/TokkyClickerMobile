...
//Rare exceptions to indicate a bug in the _tokkyclickermobile code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_tokkyclickermobile = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_tokkyclickermobile]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.tokkyclickermobile.module"
